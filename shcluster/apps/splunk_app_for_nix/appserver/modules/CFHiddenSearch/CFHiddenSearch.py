...
class CFHiddenSearch(module.ModuleHandler):
    ...
    def generateResults(self, **kwargs):
        ...
        if not sid:
            raise Exception('CFHiddenSearch.generateResults - sid not passed!')

        try:
            job = splunk.search.getJob(sid);

        except splunk.ResourceNotFound, e:
            logger.error('CFHiddenSearch could not find job %s. Exception: %s' % (sid, e))
