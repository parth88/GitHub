...
logger = \
logging.getLogger('splunk.appserver.controllers.module.CFScrollerXSimple')
...
class CFScrollerXSimple(module.ModuleHandler):

    def generateResults(self, **kwargs):
        sid = kwargs.get('sid')
        count = max(int(kwargs.get('count', 100)), 0)
        offset = max(int(kwargs.get('offset', 0)), 0)
        entity_name = kwargs.get('entity_name', 'results')

        if not sid:
            raise Exception('CFScrollerXSimple.generateResults - sid not passed!')

        try:
            job = splunk.search.getJob(sid);

        except splunk.ResourceNotFound, e:
            logger.error('CFScrollerXSimple could not find job %s. Exception: %s' % (sid, e))
