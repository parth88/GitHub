...
window.Grad = function(container, id, mode){
    var gradientContainer,
        gradient,
        self = this,
        modes = {'leftToRight':false, 'bottomUp': false},
        edgeDistance = 1.0;
    ...
    */
    function correctEdges(){
        ...
        if(modes['leftToRight']){
            edgeTarget = 'stop.left-edge';
        } else {
            edgeTarget = 'stop.right-edge';
        }

        $(gradient.node()).find(edgeTarget).each(function(){
            $el = $(this);
            ...
            if($elTarget.length){
                $el.attr('stop-color', $elTarget.attr('stop-color'));
            } else {
                if(modes['bottomUp']){
                    ...
                    // the left edge has no previous stop, 
                    // so we use the next stop's color
